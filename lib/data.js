...
export function useStaff() {
  ...
  const reload = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("staff").select("id,name,pin").eq("aktiv", true).order("sort");
    if (!error && data) {
      const sbIds = new Set(data.map((s) => s.id));
      setStaff([...data, ...local.filter((s) => !sbIds.has(s.id))]);
    } else {
      setStaff(local);
    }
    setLoading(false);
...
export async function addStaff({ name, pin }) {
  if (hasSupabase) {
    await supabase.from("staff").insert({ name, pin, aktiv: true, sort: 99 });
    return;
  }
  const list = lsGet("sw_staff", []);
  lsSet("sw_staff", [...list, { id: _uid(), name, pin, sort: list.length + 1 }]);
}

export async function removeStaff(id) {
  if (hasSupabase) {
    await supabase.from("staff").update({ aktiv: false }).eq("id", id);
    return;
  }
  lsSet("sw_staff", lsGet("sw_staff", []).filter((s) => s.id !== id));
}
...
/* ============================================================================
   Tagesabschlüsse + Buchungsoverrides (localStorage — kein Schema-Change nötig)
   sw_days        : [{id, name, startedAt, closedAt, totalRevenue, paymentCount}]
   sw_day_start   : ISO-Timestamp des aktuellen Tagesbeginns
   sw_pay_ov      : {[paymentId]: {canceled?, label?, amount?, note?}}
   ============================================================================ */
...
export function useDays() {
  ...
  useEffect(() => {
    setDays(lsGet("sw_days", []));
    setDayStart(lsGet("sw_day_start", null));
  }, []);

  const closeDay = useCallback((name, payments) => {
    const totalRevenue = payments.reduce((a, p) => a + p.amount, 0);
    const now = new Date().toISOString();
    const day = {
      id: _uid(), name,
      startedAt: lsGet("sw_day_start", null),
      closedAt: now,
      totalRevenue,
    ...
    const next = [...days, day];
    lsSet("sw_days", next);
    lsSet("sw_day_start", now);
    setDays(next);
    setDayStart(now);
  }, [days]);

  return { days, dayStart, closeDay };
}

export function usePaymentOverrides() {
  const [overrides, setOverrides] = useState({});

  useEffect(() => { setOverrides(lsGet("sw_pay_ov", {})); }, []);

  const applyOverride = useCallback((id, patch) => {
    setOverrides(prev => {
      const next = { ...prev, [id]: { ...prev[id], ...patch } };
      lsSet("sw_pay_ov", next);
      return next;
    });
  }, []);
  ...
  return { overrides, applyOverride, clearOverride };
}
